...
class NTM(nn.Module):
    ...
    def create_new_state(self, batch_size, device):
        init_r = [r.clone().repeat(batch_size, 1).to(device) for r in self.init_r]
        controller_state = self.controller.create_new_state(batch_size, device)
        heads_state = [head.create_new_state(batch_size, device) for head in self.heads]
